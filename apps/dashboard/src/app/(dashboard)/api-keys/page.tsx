...
export default function ApiKeysPage() {
  ...
  const mayManage = can('manageApiKeys');
  ...
  const create = useMutation({
    mutationFn: (input: { name: string; type: string }) =>
      api<ApiKey>(`/projects/${projectId}/api-keys`, {
        method: 'POST',
    ...
    onSuccess: async (key) => {
      setCreateOpen(false);
      setCreated(key);
      await client.invalidateQueries({ queryKey: ['api-keys', projectId] });
  ...
  const createButton = (
    <Dialog open={createOpen} onOpenChange={setCreateOpen}>
      <DialogTrigger asChild>
        <Button disabled={!mayManage} title={mayManage ? undefined : deniedReason('manageApiKeys')}>
      ...
      <DialogContent>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Server keys ingest backend logs. Client keys only ingest browser logs.
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <>
      <PageHeader
        title="API Keys"
        description="Credentials used by agents, services, and browser SDKs."
        action={createButton}
      />
      <Card>
        <CardContent>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Clients using this key will immediately stop sending logs.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
      ...
      <Dialog open={Boolean(created)} onOpenChange={(open) => !open && setCreated(undefined)}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>Copy your API key</DialogTitle>
            <DialogDescription>
              This raw key is shown once. Store it securely before closing.
            </DialogDescription>
          </DialogHeader>
          ...
          {created?.type === 'SERVER' && (
            <div className="mt-2">
              <IntegrationGuide apiKey={created.key ?? ''} />
            </div>
          )}
          <DialogFooter>
